...
const TranscriptionPanel = ({ transcriptionData = [], currentTime = 0, onJumpToTime, isTranscribing = false }) => {
  ...
  useEffect(() => {
    if (Array.isArray(transcriptionData)) {
      ...
      console.error('Invalid transcription data:', transcriptionData);
      setEditedData([]);
    }
  }, [transcriptionData]);

  // Make sure transcription data is valid before filtering
  const filteredData = Array.isArray(editedData) 
  ...
  const handleCopyAll = () => {
    if (!Array.isArray(editedData) || editedData.length === 0) return;
    
    const fullText = editedData
  ...
  const isCurrentSegment = (item) => {
    if (!item || typeof item.startTime !== 'number' || typeof item.endTime !== 'number') return false;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            <motion.button
              ...
              disabled={!Array.isArray(editedData) || editedData.length === 0}
            ...
            >
              <SafeIcon 
                icon={copiedIndex === 'all' ? FiCheckCircle : FiCopy} 
                className={`w-4 h-4 ${copiedIndex === 'all' ? 'text-green-500' : ''}`} 
              />
              {copiedIndex === 'all' && (
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {isTranscribing && (!Array.isArray(editedData) || editedData.length === 0) && (
        ...
        {(!Array.isArray(editedData) || editedData.length === 0) && !isTranscribing && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              item && item.id && (
                <motion.div
                  ...
                  className={`p-4 rounded-lg border transition-all duration-200 ${
                    isCurrentSegment(item) ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
        ...
        {searchTerm && Array.isArray(filteredData) && filteredData.length === 0 && Array.isArray(editedData) && editedData.length > 0 && (
